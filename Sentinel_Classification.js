...
//Select and Filtered Sentinel 2 L2A Image
var sentImage = ee.Image('COPERNICUS/S2_SR/20181227T055231_20181227T055233_T43SCT');

var sentClassImage = sentImage.clip(area); //clip the region of interest from the image
...
var classified = sentClassImage.select(bands).classify(cartClassifier);
//Display classification results
Map.centerObject(classNames, 13);
Map.addLayer(classified,{min: 0, max: 3, palette: ['red', 'green', 'blue','orange']},'SENTINEL CART Classification');
...
print('SENTINEL CART Accuracy:', errorMatrix.accuracy());

var classified = sentClassImage.select(bands).classify(rfClassifier);
//Display classification results
Map.centerObject(classNames, 13);
Map.addLayer(classified,{min: 0, max: 3, palette: ['red', 'green', 'blue','orange']},'SENTINEL RF Classification');

//Run the classification for test data
var validation = testing.classify(rfClassifier);
var errorMatrix = validation.errorMatrix('landcover', 'classification');
print('SENTINEL Error Matrix:', errorMatrix);
print('SENTINEL RF Accuracy:', errorMatrix.accuracy());

var classified = sentClassImage.select(bands).classify(nbClassifier);
//Display classification results
Map.centerObject(classNames, 13);
Map.addLayer(classified,{min: 0, max: 3, palette: ['red', 'green', 'blue','orange']},'SENTINEL Naive Bayes Classification');

//Run the classification for test data
var validation = testing.classify(nbClassifier);
var errorMatrix = validation.errorMatrix('landcover', 'classification');
print('SENTINEL Error Matrix:', errorMatrix);
print('SENTINEL NB Accuracy:', errorMatrix.accuracy());

var classified = sentClassImage.select(bands).classify(svmClassifier);
//Display classification results
Map.centerObject(classNames, 13);
Map.addLayer(classified,{min: 0, max: 3, palette: ['red', 'green', 'blue','orange']},'SENTINEL SVM Classification');
...
Map.addLayer(sentImage, sentbands, "Sentinel TCC");
