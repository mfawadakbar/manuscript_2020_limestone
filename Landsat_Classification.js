...
//Select and Filtered Landsat 8 Image
var landImage = ee.Image('LANDSAT/LC08/C01/T1_RT/LC08_150036_20150107');

var landClassImage = landImage.clip(area); //clip the region of interest from the image
var bands = ["B7", "B10", "B11"]; //Bands selected based on literature
...
var datasamples = landImage.select(bands).sampleRegions({
...
print('Landsat After sampling',datasamples);
...
print('LANDSAT Training',training);
print('LANDSAT Testing',testing);

// Select the CART classifier as classification algorithm
var cartClassifier = ee.Classifier.cart().train({
  features: training,
  classProperty: 'landcover',
  inputProperties: bands
});
// Select the Raindom Forrest classifier as classification algorithm
var rfClassifier = ee.Classifier.randomForest().train({
  features: training,
  classProperty: 'landcover',
  inputProperties: bands
});
// Select the SVM classifier as classification algorithm
var svmClassifier = ee.Classifier.svm().train({
  features: training,
  classProperty: 'landcover',
  inputProperties: bands
});
// Select the Naive Bayes classifier as classification algorithm
var nbClassifier = ee.Classifier.naiveBayes().train({
  features: training,
  classProperty: 'landcover',
  inputProperties: bands
});
...
var classified = landClassImage.select(bands).classify(cartClassifier);
//Display classification results
Map.centerObject(classNames, 13);
Map.addLayer(classified,{min: 0, max: 3, palette: ['red', 'green', 'blue','orange']},'LANDSAT CART Classification');
...
print('LANDSAT Error Matrix:', errorMatrix);
print('LANDSAT CART Accuracy:', errorMatrix.accuracy());

var classified = landClassImage.select(bands).classify(rfClassifier);
//Display classification results
Map.centerObject(classNames, 13);
Map.addLayer(classified,{min: 0, max: 3, palette: ['red', 'green', 'blue','orange']},'LANDSAT RF Classification');

//Run the classification for test data
var validation = testing.classify(rfClassifier);
var errorMatrix = validation.errorMatrix('landcover', 'classification');
print('LANDSAT Error Matrix:', errorMatrix);
print('LANDSAT RF Accuracy:', errorMatrix.accuracy());

var classified = landClassImage.select(bands).classify(nbClassifier);
//Display classification results
Map.centerObject(classNames, 13);
Map.addLayer(classified,{min: 0, max: 3, palette: ['red', 'green', 'blue','orange']},'LANDSAT Naive Bayes Classification');

//Run the classification for test data
var validation = testing.classify(nbClassifier);
var errorMatrix = validation.errorMatrix('landcover', 'classification');
print('LANDSAT Error Matrix:', errorMatrix);
print('LANDSAT NB Accuracy:', errorMatrix.accuracy());

var classified = landClassImage.select(bands).classify(svmClassifier);
//Display classification results
Map.centerObject(classNames, 13);
Map.addLayer(classified,{min: 0, max: 3, palette: ['red', 'green', 'blue','orange']},'LANDSAT SVM Classification');

//Run the classification for test data
var validation = testing.classify(svmClassifier);
var errorMatrix = validation.errorMatrix('landcover', 'classification');
print('LANDSAT Error Matrix:', errorMatrix);
print('LANDSAT SVM Accuracy:', errorMatrix.accuracy());
...
Map.addLayer(landImage, landbands, "Landsat TCC");
